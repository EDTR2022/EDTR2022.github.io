...
const texto = ["Amplitud ", "Periodo ", "Desfase ", "Formas de la onda", "Opciones",
    "Amplitud ", "Periodo ", "Desfase ", "Velocidad Angular", "Detener", "Continuar", "Gráfica resultante"];
const medidaTxt = [];
for (let i = 0; i < 12; i++) {
...
function onBarraBotones() {
    ...
    if (vSJAmplitud) {
        pincel.drawImage(desplazamientoTrue, CxyDesplazamiento.x - seccion / 2, CxyDesplazamiento.y - seccion / 2, seccionT,seccionT);
    }
    else {
        pincel.drawImage(desplazamientoFalse, CxyDesplazamiento.x - seccion / 2 + 2, CxyDesplazamiento.y - seccion / 2 + 2, seccionT, seccionT);
    }

    CxyVelocidad = { x: Lienzo.width / 2 - (Margen + seccion), y: AltoY + seccion / 2 };
    if (vSJPeriodo) {
        pincel.drawImage(velocidadTrue, CxyVelocidad.x - seccion / 2, CxyVelocidad.y - seccion / 2, seccionT, seccionT);
        //console.log(vSJAmplitud)
    }
    else {
        pincel.drawImage(velocidadFalse, CxyVelocidad.x - seccion / 2 + 2, CxyVelocidad.y - seccion / 2 + 2, seccionT, seccionT);
    }

    CxyAceleracion = { x: Lienzo.width / 2 + Margen + seccion, y: AltoY + seccion / 2 };
    if (vSJDesfase) {
        pincel.drawImage(aceleracionTrue, CxyAceleracion.x - seccion / 2, CxyAceleracion.y - seccion / 2, seccionT, seccionT);
    }
    else {
        pincel.drawImage(aceleracionFalse, CxyAceleracion.x - seccion / 2 + 2, CxyAceleracion.y - seccion / 2 + 2, seccionT, seccionT);
    }
    pincel.closePath();
}
...
function graficar() {
    ...
    for (let i = 0; i < 9; i++) {
        let txt = 2.0 -i*0.5;
        pincel.fillText(txt, PosX1-10, eje2_Y - CoefLineas[i] * AmplitudMax);
